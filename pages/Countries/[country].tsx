...
const Country: React.FC<{ country: string; about: any }> = (props) => {
  ...
  return (
    <motion.div
      ...
      style={{ opacity: 0, paddingTop: `${size[1] * 0.9}px` }}
      className={styles.CardParent}
    >
      <section style={{ height: `${size[1] * 0.9}px` }} className={styles.CountryMainImageWrapper}>
        <div className={styles.CountryMainImageHolder}>
          ...
          <motion.article animate={showBox ? { opacity: 1, x: ["-10%", "0%"] } : {}} className={styles.Strong}>
            ...
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
              <strong
                onClick={() => {
                  router.push("/Toures/" + props.about.Name);
                }}
                className={styles.a}
              >
                Toures
